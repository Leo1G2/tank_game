...
def draw_arrow_keys(surface, x, y, pressed_keys):
    ...
    # Touche Bas (pas utilisée dans le jeu mais dessinée pour la cohérence)
...
def run_instructions(screen):
    ...
    # État des touches pour l'animation
    pressed_keys = {}
    key_cycle = 0
    ...
    while running:
        ...
        # Vérifier si la souris est sur le bouton de retour
        back_button_hover = back_button.collidepoint(mouse_pos)
        if back_button_hover and mouse_clicked:
            return STATE_MENU
        
        # Animation des touches pour la démonstration
        key_cycle = (key_cycle + 1) % 120
        
        # Simuler des touches pressées pour l'animation
        if key_cycle < 30:
        ...
        # Titre
        title_text = font_title.render("INSTRUCTIONS", True, (255, 200, 0))
        title_shadow = font_title.render("INSTRUCTIONS", True, (150, 100, 0))
        ...
        # Dessiner les sections de contrôles
        tank1_controls = [
            ("Avancer", "↑"),
            ("Tourner à gauche", "←"),
            ("Tourner à droite", "→"),
